...
class YAMLWindow(QObject):
    ...
    def import_prj(self):
        ...
        if 'function_config' in config:
            ...
            # 将功能配置数据传递给功能配置窗口
            self.application.sub_window.function_config_window_instance.load_function_config(function_config)
    ...
    def update_function_config(self):
        ...
        for item in function_config_data:
            converted_function_config.append({
                'address': item['address'],
                'function': item['function'],
                'record': item['record'],
                'var_name': item['var_name'],
                'multi_var': item['multi_var'],
                'formula': item['formula'],
                'range_val': item['range_val'],
                'sign': item['sign']
            })
        ...
        if hasattr(self.application.sub_window, 'add_data_window_instance'):
            # 如果实例已存在，直接更新
            self.application.sub_window.add_data_window_instance.update_combo_box_address(self.file_path)
