...
class SpecialLog():
    ...
    def add_error_message(self, error_round, write_address, write_data, read_address, read_data, packet_type, hw_count=None, write_base_address=None, read_base_address=None):
        ...
        Args:
            ...
            hw_count: 写入半字次数（int，可选）
            write_base_address: 写基地址（str，十六进制，可选）
            read_base_address: 读基地址（str，十六进制，可选）
        """

        # 根据参数是否存在构建错误消息
        if hw_count is not None and write_base_address is not None and read_base_address is not None:
            # 使用新格式（包含新增字段）
            error_message = f"错误出现轮次:{error_round},写入半字次数:{hw_count},写基地址:{write_base_address},写数据地址:{write_address},写数据:{write_data},读基地址:{read_base_address},读数据地址:{read_address},读数据:{read_data},测试类型:{packet_type}"
        else:
            # 使用旧格式（兼容旧调用）
    ...
    def export_pdf(self, filename="log_export.pdf"):
        ...
        while i < len(self.message_list):
            ...
            if "运行结果出现错误" in content:
                ...
                # 检查是否包含新字段，以确定表头
                next_msg = self.message_list[i+1] if i+1 < len(self.message_list) else ""
                next_content = next_msg.split('  ', 1)[1] if '  ' in next_msg else next_msg
                
                # 根据是否包含新字段确定表头
                if '写入半字次数:' in next_content and '写基地址:' in next_content and '读基地址:' in next_content:
                    # 新格式表头
                    table_headers = ["错误出现轮次", "写入半字次数", "写基地址", "写数据地址", "写数据", "读基地址", "读数据地址", "读数据", "测试类型"]
                else:
                    # 旧格式表头
