...
class CaseItemWidget(QWidget):
    ...
    send_frame = Signal(bytearray, bool)
    ...
    def init_widget(self, button = True, delay = False):
        ...
        - 测试模式：包含名称标签、输入框和延时输入框及删除按钮
        
        Args:
            button (bool): 是否显示按钮，默认为True
            delay (bool): 是否显示延时输入框，默认为False
        """
         # 创建水平布局
        ...
        if self.mode == "send":
            ...
            self.delay_input.setPlaceholderText("发送延时(s),默认3秒")
