...
from .test_group_manager import GroupManager
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.ui.button_add_data_group.setVisible(False)
        self.ui.button_del_data_group.setVisible(False)
        ...
        self.ui.button_message.clicked.connect(
        ...
        self.group_manager = GroupManager(self.ui.combo_box_mode_group, self.ui.list_group, self.ui.list_data, self)

        self.ui.button_add_mode_group.clicked.connect(self.group_manager.add_mode_group)
        self.ui.button_del_mode_group.clicked.connect(self.group_manager.delete_mode_group)
        self.ui.combo_box_mode_group.currentIndexChanged.connect(self.group_changed)

        self.ui.button_del_select.clicked.connect(self.group_manager.delete_mode_group_item)
    ...
    def group_changed(self, index):
        ...
        self.group_manager.load_mode_group_data(new_group)
    ...
    def on_name_updated(self, new_name, index=None):
        if index is not None and 0 <= index < self.ui.combo_box_data_group.count():
            old_name = self.ui.combo_box_data_group.currentText()

            self.ui.combo_box_data_group.setItemText(index, new_name)
            if hasattr(self, 'yaml_config') and old_name != new_name:
                self.yaml_config.rename_sync(old_name, new_name)
                self.yaml_config.schedule_auto_save()
    ...
    def sending_mode(self):
        ...
        if self.ui.check_box_mode_poll.isChecked():
            """            
                ...
                Intra-group is fixed at 0.5 seconds.
            """
            data_poll = True

            # Add all items in all groups to item_send
            all_data = self.group_manager.get_data_group()
            ...
            # only add items in list_group to item_send
