...
class YamlConfig(QObject):
    ...
    def setup_auto_save_connections(self):
        ...
        self.ui.list_data.model().rowsInserted.connect(self.schedule_auto_save)
        self.ui.list_data.model().rowsRemoved.connect(self.schedule_auto_save)
        self.ui.list_data.model().rowsMoved.connect(self.on_rows_moved)

        self.ui.list_group.model().rowsRemoved.connect(self.schedule_auto_save)
    ...
    def import_folder(self):
        try:
            ...
            for group_path in data_group_paths:
                ...
                full_group_path = os.path.join(folder_path, 'group', group_path)
    ...
    def load_data_group_config(self, config):
        try:
            ...
            if 'mode_group' in config and self.group_manager:
                mode_group = config['mode_group']
                if not isinstance(mode_group, dict):
                    mode_group = {}
                self.group_manager.set_group_data(mode_group)
            
            # self.log("数据分组配置加载成功", 1)
        except Exception as e:
    ...
    def import_config(self):
        try:
            ...
            if 'data_group_name' in config:
                ...
                data_group_name = config['data_group_name']
                # if isinstance(data_group_name, list):
                #     data_group_name = ', '.join(str(item) for item in data_group_name)
                # else:
                #     data_group_name = str(data_group_name)

                self.ui.combo_box_data_group.addItem(data_group_name)

            if 'data_group' in config:
                self.ui.list_data.clear()
                self.data_map.clear()
                
                for item in config['data_group']:
                    if 'name' in item and 'data' in item:
                        if hasattr(self.parent, 'mainwindow_data'):
                            self.parent.mainwindow_data(
                                item['name'], item['data']
                            )
            ...
            if 'vcc' in spi_config:
                ...
                if vcc_index >= 0:
                    ...
                    self.log(f"未找到VCC值: {vcc_value}", 2)
            ...
            if 'speed' in spi_config:
                ...
                if speed_index >= 0:
                    ...
                    self.log(f"未找到Speed值: {speed_value}", 2)
            ...
            if 'clk' in spi_config:
                ...
                if clk_index >= 0:
                    ...
                    self.log(f"未找到CLK值: {clk_value}", 2)
            ...
            if 'bit' in spi_config:
                ...
                if bit_index >= 0:
                    ...
                    self.log(f"未找到Bit值: {bit_value}", 2)

            if 's_or_q' in spi_config:
                s_or_q_value = str(spi_config['s_or_q'])
                s_or_q_index = self.ui.combo_box_s_or_q.findText(s_or_q_value)
                if s_or_q_index >= 0:
                    self.ui.combo_box_s_or_q.setCurrentIndex(s_or_q_index)
                else:
                    self.log(f"未找到S或Q值: {s_or_q_value}", 2)
            ...
            if 'rx_size' in spi_config:
                ...
                if rx_size_index >= 0:
                    ...
                    self.log(f"未找到读数据长度值: {rx_size_value}", 2)
            ...
            self.log(f"配置文件导入成功: {os.path.basename(file_path)}", 1)
    ...
    def export_config(self):
        ...
        if self.group_manager:
            ...
            raw_group_data = self.group_manager.get_group_data()
    ...
    def auto_save_config(self):
        if self.temp_auto_save or not self.current_config_path or not os.path.exists(self.current_config_path):
            return
