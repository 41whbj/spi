...
class PC_MCU2Window(QObject):
    ...
    def send_frame(self, frame, correct):
        ...
        QTimer.singleShot(int(delay_value * 1000), self.case_result_received)

    def case_result_received(self):
        if not hasattr(self.main_window, 'spi_device') or not self.main_window.spi_device:
            self.main_window.log("SPI设备未连接", 2)
            return
        ...
        if cmd == (CMD["Nack"]):
            self.main_window.log("接收到Nack命令字,发生错误", 2)
        ...
        if len(payload) < 2:
            self.main_window.log("负载数据长度不足", 2)
            return
        ...
        if payload[2:4] != b'\x00\x00':
            ...
            for case_name, case_payload in processed_result.items():
                if payload[:2]== case_payload:
                    case_message = f"{case_name}运行结果出现错误"
                    self.main_window.log(case_message, 2)
                    # print(case_message)

            self.log.add_message(case_message)
            ...
            for case_name, case_payload in processed_result.items():
                if payload[:2]== case_payload:
                    message = f"{case_name}运行结果正确"
                    # self.main_window.log(f"{case_name}运行结果正确", 1)
                    # print(message)
                    self.log.add_message(message)
                    break
