...
date: 2025-10-29
description: PC-MCU data frame related functions
"""

from crc_manager.crc import CRC
from log_manager.log import LogManager
...
CMD = {
    ...
    "Getlog": 0x0003, # PC request log from MCU
...
class Frame():
    ...
    def generate_frame(cmd, data=None, use_crc=False):
        global current_msg_id
        # Get the current id and increse it
        ...
        if data is None:
            ...
            if isinstance(data, bytes):
                ...
                # Data is not bytes, convert it to bytes
                payload = bytes(data)
        ...
        if use_crc:
            crc_value = CRC.crc_16_user(frame)
            frame += crc_value.to_bytes(2, byteorder='big')

        log.frame_record(msg_id, cmd, data)
    ...
    def parse_receive_frame(receive_data, use_crc=False):
        ...
        if len(receive_data) < expected_length:
            ...
            print(f"len(receive_data): {len(receive_data)}")    
            return False, None, None, None, "data length error"
    ...
    def parse_normally(payload):
        try:
            # Decode from bytes to ascii string
            str = payload.decode('ascii')

            # print(f"case_list: {str}")
