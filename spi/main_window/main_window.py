...
from test_group.testgroup2window import TestGroupManager
...
class MainWindow(QWidget):
    ...
    def mainwindow_data(self, data_name, data_text, show_all=True):
        ...
        list_item_widget.send_clicked_signal.connect(self.spi_send_item)
        list_item_widget.data_changed_signal.connect(self.item_data_change)
    ...
    def item_data_change(self, item, new_text):
        ...
        updated_data = (original_data[0], new_text) + original_data[2:]
        item.setData(Qt.UserRole, updated_data)
        
        # 更新data_map中的数据
        if hasattr(self, 'data_map') and original_data[0] in self.data_map:
            self.data_map[original_data[0]] = new_text
        
        # 更新CRC提示
        self.update_all_crc_tooltips()
        
        # 触发自动保存
        if hasattr(self.data_group_manager, 'check_and_save_data'):
            self.data_group_manager.check_and_save_data()

        print('test1')

        current_group_name = self.data_group_manager.get_current_group_name()
        if current_group_name and current_group_name in self.data_group_manager.group_manager:
            group_data = self.data_group_manager.group_manager[current_group_name]['data']
            for i, data_item in enumerate(group_data):
                if isinstance(data_item, tuple) and len(data_item) >= 2:
                    if data_item[0] == original_data[0]:  # 匹配名称
                        # 更新group_manager中的数据
                        self.data_group_manager.group_manager[current_group_name]['data'][i] = updated_data
                        print(f"更新组 {current_group_name} 中的数据项 {i}: {updated_data}")
                        break
...
# Handle window close event
    def closeEvent(self, event):
        ...
        elif self.ui.stacked_widget.currentIndex() == 1:
                        # 显示提示对话框
            reply = QMessageBox.question(
                self, 
                '关闭确认', 
    ...
    def init_spi_monitoring(self):
        ...
        self.device_monitor_timer.timeout.connect(self.monitor_spi_devices)
    ...
    def handle_device_connected(self, device_count):
        ...
        if not hasattr(self, 'spi_device') or self.spi_device is None:
            ...
            print(self.spi_device)
        
        if self.spi_device.open_device() and not self.content_initialized:
            device_name = self.spi_device.get_device_name()
            self.spi.spi_combo_box()
            # 确保spi_clk_mode和spi_bit_order变量正确初始化
            self.spi_clk_mode = self.ui.combo_box_clk.currentIndex()
            self.spi_bit_order = self.ui.combo_box_bit.currentIndex()
    ...
    def handle_device_disconnected(self):
        ...
        self.spi.clear_combo_boxes()
        self.ui.line_device.clear()
        self.spi.widget_disconnect()
        self.content_initialized = False
