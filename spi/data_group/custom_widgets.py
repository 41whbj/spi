...
class User_ListItemWidget(QWidget):
    ...
    def init_ui(self, data_name, data_text, show_all, checkable, sendable):
        ...
        if show_all:
            ...
            self.data_text.editingFinished.connect(self.on_edit_finish)
        else:
        ...
        #     self.h_layout.setStretch(self.h_layout.indexOf(self.name_text), 1)
        #     self.h_layout.setStretch(self.h_layout.indexOf(self.data_label), 3)
        else:
    ...
    def on_edit_finish(self):
        ...
        self.data_changed.emit(getattr(self, 'user_data', None), formatted_text)
