...
class SendModeThread(QThread):
    ...
    def send_item_data(self, item):
        ...
        # print(self.parent.spi_device.dev_handle)
        # print(f"发送时的时钟索引: {self.parent.spi_clk_mode}")
        # print(f"发送时的位序索引: {self.parent.spi_bit_order}")
        ...
        if self.parent.ui.check_box_test.isChecked():
            received_data = self.parent.spi_device.spi_transfer(
                [int(part, 16) for part in hex_parts], 
                self.parent.spi_clk_mode,
                self.parent.spi_bit_order
            )

            self.log_signal.emit(f"接收,{received_data.hex()}", 1)