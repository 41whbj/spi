...
class DataGroupManager(QObject):
    ...
    def add_new_name(self,new_name):
        ...
        # print(f"当前数据组管理器: {self.group_manager}")
    ...
    def name_updated(self,new_name, old_name, index=None):
        ...
        if index is not None and 0 <= index < self.ui.combo_box_data_group.count():
            ...
            # print(f"name_updated当前数据组管理器: {self.group_manager}")
    ...
    def add_data_widget(self, data_name, data_text, show_all=True):
        """
        ...
        Args:
            ...
            show_all (bool): 是否显示所有数据项，默认为True
        """
        ...
        if current_group_name in self.group_manager:
            ...
            existing_data = self.group_manager[current_group_name]['data']
        ...
        if current_group_name in self.group_manager:
            # 将新的数据项添加到当前组的数据列表中
            self.group_manager[current_group_name]['data'].append((data_name, format_text))

        # print(f"添加数据后的group_manager：{self.group_manager}")
        ...
        list_item_widget.add_clicked_signal.connect(self.item_add_clicked)
    ...
    def item_data_changed(self, item, new_data):
        ...
        # 检查当前选中的数据组是否存在
        if current_group_name and current_group_name in self.group_manager:

            # 获取当前选中的数据组中的数据
            group_data = self.group_manager[current_group_name]['data']
            ...
            for i, data_item in enumerate(group_data):
                ...
                if data_item[0] == original_data[0]:
                    ...
                    # 更新group_manager中的数据
                    self.group_manager[current_group_name]['data'][i] = updated_data
                    
                    # print(f"更新后的group_manager数据: {self.group_manager}")
        ...
        for item in selected_items:
            ...
            if current_group_name and current_group_name in self.group_manager:
                ...
                if item_data:
                    ...
                    self.group_manager[current_group_name]['data'] = [
                        data for data in self.group_manager[current_group_name]['data'] 
