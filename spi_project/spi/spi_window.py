...
class SPIWindow(QObject):
    ...
    def __init__(self, application, driver=None):
        ...
        self.device_detection_timer.start(1000)  # 每1秒检测一次
    ...
    def set_combox_index(self):
        ...
        clk_mode = self.ui.combo_box_clk.currentText()
        bit_order = self.ui.combo_box_bit.currentText()
        s_or_q = self.ui.combo_box_s_or_q.currentText()
        size = self.ui.combo_box_size.currentText()

        for i in range(self.ui.combo_box_vcc.count()):
            if self.ui.combo_box_vcc.itemText(i) == vcc:
                self.ui.combo_box_vcc.setCurrentIndex(i)
                # print(f"vcc: {self.ui.combo_box_vcc.currentText()}")
                break
        
        # 对于IO下拉框
        for i in range(self.ui.combo_box_io.count()):
            if self.ui.combo_box_io.itemText(i) == io:
                self.ui.combo_box_io.setCurrentIndex(i)
                # print(f"io: {self.ui.combo_box_io.currentText()}")
                break
        
        # 对于速度下拉框
        for i in range(self.ui.combo_box_speed.count()):
            if self.ui.combo_box_speed.itemText(i) == speed:
                self.ui.combo_box_speed.setCurrentIndex(i)
                # print(f"speed: {self.ui.combo_box_speed.currentText()}")
                break
        
        # 对于时钟模式下拉框
        for i in range(self.ui.combo_box_clk.count()):
            if self.ui.combo_box_clk.itemText(i) == clk_mode:
                self.ui.combo_box_clk.setCurrentIndex(i)
                # print(f"clk_mode: {self.ui.combo_box_clk.currentText()}")
                break
        
        # 对于位序下拉框
        for i in range(self.ui.combo_box_bit.count()):
            if self.ui.combo_box_bit.itemText(i) == bit_order:
                self.ui.combo_box_bit.setCurrentIndex(i)
                # print(f"bit_order: {self.ui.combo_box_bit.currentText()}")
                break
        
        # 对于SPI模式下拉框
        for i in range(self.ui.combo_box_s_or_q.count()):
            if self.ui.combo_box_s_or_q.itemText(i) == s_or_q:
                self.ui.combo_box_s_or_q.setCurrentIndex(i)
                # print(f"s_or_q: {self.ui.combo_box_s_or_q.currentText()}")
                break
        
        # 对于数据长度下拉框
        for i in range(self.ui.combo_box_size.count()):
            if self.ui.combo_box_size.itemText(i) == size:
                self.ui.combo_box_size.setCurrentIndex(i)
                # print(f"size: {self.ui.combo_box_size.currentText()}")
                break
