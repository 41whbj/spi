...
class SPIController(QObject):
    ...
    def spi_send(self, data, clk_mode, bit_order, log = True):
        """
        发送SPI数据
        ...
        # 检查SPI设备是否已连接
        if self.driver.dev_handle is None:
            # print(f"SPIController:dev_handle: {self.driver.dev_handle}")
            QMessageBox.warning(None, "警告", "SPI设备未连接")
            return
    ...
    def spi_receive(self, clk_mode, bit_order, rx_size, log = True):
        ...
        if self.driver.jtool is None or self.driver.dev_handle is None:
            QMessageBox.warning("SPI设备未连接", 2)
            return
